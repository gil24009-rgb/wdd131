...
function renderTags(tags = []) {
  ...
  const tagsHtml = safeTags
    .map((tag) => `<span class="tag" aria-label="Category: ${escapeHtml(tag)}">${escapeHtml(tag)}</span>`)
    .join("");
...
function renderRatingStars(rating) {
  ...
  for (let i = 1; i <= total; i++) {
    if (i <= full) out += "⭐";
    else if (hasHalf && i === full + 1) out += "⭐";
    else out += "☆";
  }

  const labelValue = Math.round(value * 10) / 10;

  return `
    <span class="rating" role="img" aria-label="Rating: ${labelValue} out of 5 stars">
...
function renderRecipeCard(recipe) {
  ...
  return `
    <article class="card" aria-label="${escapeHtml(name)} recipe">
      <img
        class="card-image"
        src="${escapeAttr(img)}"
        alt="${escapeAttr(name)}"
      ...
      <div class="card-body">
        ...
        <h2 class="card-title">
          <a class="card-link" href="#" aria-label="Open recipe: ${escapeAttr(name)}">${escapeHtml(name)}</a>
        </h2>
...
function renderRecipes(list, statusText) {
  recipesEl.innerHTML = list.map(renderRecipeCard).join("");
  setStatus(statusText);
}

function renderRandomRecipe() {
  const idx = getRandomIndex(recipes.length);
  renderRecipes([recipes[idx]], "Showing a random recipe.");
}
...
function renderSearchResults(query) {
  ...
  const filtered = recipes.filter((r) => recipeMatchesQuery(r, q)).sort(compareByName);

  if (filtered.length === 0) {
    recipesEl.innerHTML = "";
    setStatus(`No recipes found for "${query}".`);
    return;
  }

  renderRecipes(filtered, `Found ${filtered.length} recipe(s) for "${query}", sorted by name.`);
}
